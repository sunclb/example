...
def generate_reports():
    ...
    user_result_summary.to_csv(os.path.join(dir,'user_result_summary.csv'))
    def box_group(ax,df,title=None):
        ...
        ax.autoscale_view()
        return ax
    fig, axs = plot.subplots(nrows=10, ncols=1,figsize=(10, 40))
    ...
    for j,value in enumerate(user_result_summary.columns[-5:-1]):
        draw_df=user_result_summary.pivot(index='date',
                                        columns='username',values=value)
        box_group(axs[j+5],draw_df,title=value)
    axs[9].axis("off")
    axs[9].table(cellText=user_result_summary.values,
